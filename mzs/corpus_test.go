		...
		tests := []struct {
			...
			{"dicts", "examples/06_dicts_records.mzs", nil, "",
				[]string{"deep merge keeps them", `"port":443`, "headcount by dept",
					`mixed keys: 4 entries, [1] is "int key"`}},
			{"functions", "examples/07_functions_closures.mzs", nil, "",
				[]string{"quicksort: [1,2,3,4,7,8,9]", "mutual recursion", "DONE!",
					"dispatch table: circle(2)=12.57", "called where it stands: 144",
					"arrow form: perimeter(3, 4)=14"}},
			...
			{"records", "examples/37_records.mzs", nil, "",
				[]string{"type(m)            → Money",
					"m.is(\"dict\")       → true",
					"Money has no parameter named 'curency'; it takes 'amount' and 'currency'",
					"cart 1043 for ann, 2 line(s)",
					"cart.total         → 12.00 RUB",
					"charge takes a Money, got dict",
					"Page(3, \"abc\").len → 3"}},
			{"heredoc", "examples/38_heredoc.mzs", nil, "",
				[]string{" WHERE name = 'Ann'",
					"your balance is $balance",
					"trimmed and upper → A QUIET SHOUT",
					"GET /orders   | 200 OK",
					`"Summary\n\n  • two spaces deeper than the rest`,
					"│ order #0002                  │"}},
			{"sets", "examples/39_sets.mzs", nil, "",
				[]string{`editors.union(admins)              → ["ann","bob","cyd","dan"]`,
					"[1,1,2] + [2]                      → [1,1,2,2]",
					"[1,1,2].union([2])                 → [1,2]",
					`reachable(graph, "a")              → ["a","b","c","d"]`,
					"try pairs.to_set                   → \"type: dict key must be hashable, got array\"",
					"новые              жалобы"}},
			{"lazy sequences", "examples/40_lazy_sequences.mzs", nil, "",
				[]string{"(1..1_000_000_000).seq.find { it * it > 500 }  → 23",
					"counted.take(3).array                          → [0,1,2]   pulls: 3",
					`paged.take(2).array  (the same expression)     → ["page 3","page 4"]`,
					`(1..3).seq.is("array")                         → false`,
					`try [3,1,2].seq.sort                           → "sort expects an array, got seq"`,
					"2026-08-17 ERROR request 302 took 302ms"}},
			{"decimal money", "examples/41_decimal_money.mzs", nil, "",
				[]string{`decimal.plus(0.1, 0.2)                               → 0.3`,
					"итого без НДС" + strings.Repeat(" ", 43) + "18698.42",
					`decimal.split(к оплате, 3, 2)                        → ["7479.37","7479.37","7479.36"]`,
					"decimal.div(1, 3)                                    → decimal: decimal.div: 1 / 3 has no exact decimal form",
					"price + vat                                          → type: cannot add Decimal to Decimal",
					`"total": "22438.10"`}},
			{"crypto webhook", "examples/43_crypto_webhook.mzs", nil, "",
				[]string{`crypto.sha256("abc")                           → ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad`,
					`crypto.base64("?~~~ ok", "url")                → P35-fiBvaw`,
					`all four spellings decode                      → ["Hi","Hi","~~~?","~~~?"]`,
					`crypto.equal(signed, header)                   → true`,
					`the body changed by one digit                  → false`,
					`crypto.crc32("123456789").str(16)              → cbf43926`,
					`type(hash("abc"))                              → int`}},
			{"urls", "examples/44_urls.mzs", nil, "",
				[]string{`  "path": "/v1/счета",`,
					`u["port"]                                        → 8443`,
					`build(parse(u)) is the same URL                  → https://ivan:s3cret@api.example.com:8443/v1/%D1%81%D1%87%D0%B5%D1%82%D0%B0?page=2&sort=desc#top`,
					`url.parse_query("q=a+b")["q"]                    → a b`,
					`what this module wrote comes back                → a+b`,
					`parse_query("?q=a+b&tag=x&tag=y&flag")           → {"q":"a b","tag":"x","flag":""}`,
					`the callback verifies                            → true`,
					`with one character changed                       → false`}},
...
func TestDiagnostics(t *testing.T) {
	...
	tests := []struct {
		...
		{"a colon after a literal key", `{1: "A"}`,
			`a dict key that is not a string takes '->', not ':'`, 1, 3},
		{"an arrow function without braces", `f = (x) -> x * 2`,
			"an arrow function's body is braced: (x) -> { x * 2 }, or write the closure { (x) -> x * 2 }", 1, 9},
		{"async in front of an arrow", `f = async (x) -> { x }`,
			"an async function is written `async fn(a, b) { … }`", 1, 5},
